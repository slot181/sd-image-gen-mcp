...
import {
  ...
  McpError,
  CallToolRequest, // Import CallToolRequest type
...
// --- Tool Imports ---
import { generateImageSchema, handleGenerateImage, ValidatedGenerateImageArgs } from './tools/generateImage.js';
import { getModelsSchema, handleGetModels, ValidatedGetModelsArgs } from './tools/getModels.js';
import { setModelSchema, handleSetModel, ValidatedSetModelArgs } from './tools/setModel.js';
import { getUpscalersSchema, handleGetUpscalers, ValidatedGetUpscalersArgs } from './tools/getUpscalers.js';
import { getSamplersSchema, handleGetSamplers, ValidatedGetSamplersArgs } from './tools/getSamplers.js';
import { getLorasSchema, handleGetLoras, ValidatedGetLorasArgs } from './tools/getLoras.js';
import { upscaleImagesSchema, handleUpscaleImages, ValidatedUpscaleImagesArgs } from './tools/upscaleImages.js';
...
const server = new Server(
  { name: 'sd-image-gen-mcp', version: '1.1.4' }, // Updated version after refactor
  { capabilities: { tools: {} } }
);
...
const toolDefinitions = [
  {
    ...
    inputSchema: { // Manually define for ListTools, validation will use Zod schema
      type: 'object',
      properties: {
...
server.setRequestHandler(CallToolRequestSchema, async (request: CallToolRequest) => {
  ...
  const toolValidationSchemas: { [key: string]: z.ZodType<any, any> } = {
  ...
  try {
    switch (name) {
      case 'generate_sd_image':
        return await handleGenerateImage(validatedArgs as ValidatedGenerateImageArgs, axiosInstance);
      case 'get_sd_models':
        return await handleGetModels(validatedArgs as ValidatedGetModelsArgs, axiosInstance);
      case 'set_sd_model':
        return await handleSetModel(validatedArgs as ValidatedSetModelArgs, axiosInstance);
      case 'get_sd_upscalers':
        return await handleGetUpscalers(validatedArgs as ValidatedGetUpscalersArgs, axiosInstance);
      case 'get_sd_samplers':
        return await handleGetSamplers(validatedArgs as ValidatedGetSamplersArgs, axiosInstance);
      case 'get_sd_loras':
        return await handleGetLoras(validatedArgs as ValidatedGetLorasArgs, axiosInstance);
      case 'upscale_sd_images':
        return await handleUpscaleImages(validatedArgs as ValidatedUpscaleImagesArgs, axiosInstance);
      default:
