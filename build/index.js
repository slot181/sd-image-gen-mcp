...
class ImageGenServer {
    ...
    async uploadToCfImgbed(imageData, filename) {
            ...
            try {
                switch (request.params.name) {
                    ...
                    case 'get_sd_samplers': {
                        ...
                        const samplers = Array.isArray(response.data) ? response.data.map((s) => s?.name).filter(Boolean) : [];
                    ...
                    case 'upscale_sd_images': {
                        ...
                        if (!response.data.images?.length)
                            throw new Error('No images upscaled');
                        const results = [];
                ...
                if (axios.isAxiosError(error)) {
                    throw new McpError(ErrorCode.InternalError, error.response ? `API error: ${error.response.data?.error || error.message}` :
...
function isGenerateImageArgs(value) {
    ...
    if (typeof v.prompt !== 'string')
        return false;
    if (v.negative_prompt !== undefined && typeof v.negative_prompt !== 'string')
        return false;
...
function isSetModelArgs(value) {
    if (typeof value !== 'object' || value === null)
        return false;
    const v = value;
    return typeof v.model_name === 'string';
}
function isUpscaleImagesArgs(value) {
    if (typeof value !== 'object' || value === null)
        return false;
    const v = value;
    ...
    if (v.upscaler_1 !== undefined && typeof v.upscaler_1 !== 'string')
        return false;
    if (v.upscaler_2 !== undefined && typeof v.upscaler_2 !== 'string')
        return false;
    if (v.output_path !== undefined && typeof v.output_path !== 'string')
        return false;
    return true;
}
