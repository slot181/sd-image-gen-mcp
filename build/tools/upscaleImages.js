import axios from 'axios'; // Import main axios for downloading
import path from 'path';
import fs from 'fs';
import { randomUUID } from 'crypto'; // For generating temp filenames
import { z } from 'zod';
import { DEFAULT_OUTPUT_DIR, SD_RESIZE_MODE, SD_UPSCALE_MULTIPLIER, SD_UPSCALE_WIDTH, SD_UPSCALE_HEIGHT, SD_UPSCALER_1, SD_UPSCALER_2, CF_IMGBED_UPLOAD_URL, // For checking if upload is configured
...
function sanitizeFilenameComponent(name, maxLength = 50) {
    if (!name) {
...
export async function handleUpscaleImages(args, axiosInstance) {
    ...
    try {
        ...
        const encodedImages = await Promise.all(processedImagePaths.map(async (filePath) => {
